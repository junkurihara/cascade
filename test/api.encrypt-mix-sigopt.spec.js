...
import mixProcedureHybridNoSig from './procs/procedure_mix-hybrid-nosig.js';
...
describe(`${env}: Encryption test with mixed encryption algorithms using OpenPGP and JSCU without signatures`, () => {
  let msg;
  let pemKeys;
  before(async () => {
  ...
  it('Mixed Hybrid: message should be successfully encrypted and decrypted', async function (){
    ...
    const encrypted = await core.encryptSeq({message: msg, keys: sortedKeys, procedureConfig: mixProcedureHybridNoSig.procedure});
